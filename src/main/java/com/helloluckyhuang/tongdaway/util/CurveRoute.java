...
//            // 计算控制手柄长度
        ...
        private static double determineHandleLength(Vec3 end1, Vec3 end2, Vec3 axis1, Vec3 axis2) {
            ...
            if (Mth.equal(angle, 0)) {
                double[] intersect = intersect(end1, end2, axis1, cross2, Direction.Axis.Y);
                if (intersect != null) {
            ...
            double[] intersect = intersect(end1, end2, cross1, cross2, Direction.Axis.Y);
    ...
    public static CurveRoute fromNBT(ListTag curveTag) {
        ...
        for (int i = 0; i < curveTag.size(); i++) {
            ...
            if (parameters.size() == 5) {
                Vec3 start = nbt2Vec((StringTag) parameters.get(0));
                Vec3 end = nbt2Vec((StringTag) parameters.get(1));
                String biome = parameters.getString(2);
                String type = parameters.getString(3);
                String note = parameters.getString(4);
                curve.addSegment(new LineSegment(start, end, biome, type, note));
            } else if (parameters.size() == 7) {
                Vec3 p0 = nbt2Vec((StringTag) parameters.get(0));
                Vec3 p1 = nbt2Vec((StringTag) parameters.get(1));
                Vec3 p2 = nbt2Vec((StringTag) parameters.get(2));
                Vec3 p3 = nbt2Vec((StringTag) parameters.get(3));
                String biome = parameters.getString(4);
                String type = parameters.getString(5);
                String note = parameters.getString(6);
    ...
    private static StringTag vec2NBT(Vec3 point) {
        String tagString = point.x + "," + point.y + "," + point.z;
        return StringTag.valueOf(tagString);
    }

    private static Vec3 nbt2Vec(StringTag pointTag) {
        String[] parts = pointTag.getAsString().split(",");
        try {
            ...
            Tongdaway.LOGGER.error("Invalid Vec3 format: {}", pointTag.getAsString());
