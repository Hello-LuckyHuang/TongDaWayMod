...
//            TongDaWay.LOGGER.info(" Region {} Build HeightMap time: {}ms", regionPos, endTime - startTime);
    ...
    private Pair<int[][], List<Pair<String, BlockPos>>> getStructureMap(WorldGenRegion level, RegionPos regionPos) {
        ...
        LinkedBlockingQueue<Runnable> linkedBlockingQueue = new LinkedBlockingQueue<Runnable>(); //线程池
        ThreadPoolExecutor executor = new ThreadPoolExecutor(16, 2048, 1, TimeUnit.DAYS, linkedBlockingQueue);

        for (int gx = 0; gx < CHUNK_GROUP_SIZE; gx++) {
            for (int gz = 0; gz < CHUNK_GROUP_SIZE; gz++) {
                var protoChunk = new ProtoChunk(new ChunkPos(regionPos.x() * CHUNK_GROUP_SIZE + gx, regionPos.z() * CHUNK_GROUP_SIZE + gz), UpgradeData.EMPTY, levelHeightAccessor, biomeRegistry, null);
